...
class IRSIMEnv(gym.Env):     #继承了 gym.Env
    ...
    def step(self, action):   # 执行一步动作，更新环境状态，计算奖励，判断是否终止，并返回新的观测序列、奖励、done标志和 info
        ...
        # for id in range(self.robot_number):
        #     obs_single = obs_all[id]  # 单机器人当前帧观测 [obs_dim]
        #     print(type(obs_single), obs_single.shape)

        #     self.obs_history[id].append(obs_single)
            
        #     # 保持历史长度为 seq_len
        #     if len(self.obs_history[id]) > self.seq_len:
        #         self.obs_history[id].pop(0)
        ...
        for id in range(self.robot_number):
            ...
            collided = self.env.robot_list[id].done() and not reached_goal   #####碰撞后应该进行归位的？？？？  
...
if __name__ == "__main__":
   ...
   #4.训练环节
    ...
    while current_timestep < total_timesteps:
        ...
        should_reset = any(info_total) or all(done_total)
        if should_reset:
